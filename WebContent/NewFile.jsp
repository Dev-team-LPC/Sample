...
<html>
    ...
    <body>
        <%!public static String report_id, mentor, months, sla_id, creationDate, learnrCount = "", sla_name = "", reportType,
                    ...
                    plan = "", placement = "", achievements = "", activity_date = "", activity_due_date = "", quarter, 
        ...
        <%
            ...
            try {
                ...
                st.executeQuery("SELECT sla.id, sla.type, sla.Name, report_date, sla.start_date, sla.end_date, if(NOW() <= DATE_ADD(start_date, INTERVAL 3 MONTH), 'First Quarter', if(NOW() <= DATE_ADD(start_date, INTERVAL 6 MONTH), 'Second Quarter', If(NOW() <= DATE_ADD(start_date, INTERVAL 9 MONTH), 'Third Quarter', if(NOW() <= DATE_ADD(start_date, INTERVAL 12 MONTH), 'Fourth Quarter', 'null')))) quarter, sla_company_details.company_name, sla_report_type.type, sla_company_details.address, sla_project_manager.name, sla_project_manager.surname, cellphone, telephone, email, introduction, methodology, methodology_details, strategic_plan, work_placement, achievements, sla_report_status.id,  CONCAT(representative_employer_name, ' ', representative_employer_surname) mentor, sla_reports_final.placement FROM sla_reports INNER JOIN sla_reports_final ON sla_reports_final.report_id = sla_reports.id INNER JOIN sla_reports_learner_tasks ON sla_reports_learner_tasks.report_id = sla_reports.id INNER JOIN sla ON sla.id = sla_reports.sla_id INNER JOIN sla_company_details ON sla_company_details.id = sla.company_id INNER JOIN sla_project_manager ON sla_project_manager.id = sla_company_details.project_manager_id INNER JOIN sla_report_status ON sla_report_status.id = sla_reports.report_status_id INNER JOIN sla_report_type ON sla_report_type.id = sla_reports.report_type_id INNER JOIN sla_reports_learner_placement ON sla_reports_learner_placement.final_report_id = sla_reports_final.id WHERE sla_reports.id = "+report_id+";");
                ...
                while (rs.next()) {
                    ...
                    status = (String) rs.getString("sla_report_status.id");
                    reportType = (String) rs.getString("sla_report_type.type");
                    mentor = (String) rs.getString("mentor");
                    location = (String) rs.getString("sla_reports_final.placement");
                }
                                        ...
                                        <table
                                            ...
                                            <tbody>
                                                ...
                                                try {
                                        			...
                                        			while(rs.next()) {
                                        				i++;
	                                                    String dueDate =  (String) rs.getString("task_due_date");
	                                                    String activityDate = (String) rs.getString("task_date"); 
	                                                    String activityName = (String) rs.getString("task_name");
	                                                    String activityOutcome = (String) rs.getString("task_outcome");
	                                                    String requiredAction = (String) rs.getString("task_action_required");
                                                        out.print("<tr>");
                                                        out.print("<td>" + i + "</td>");
                                                        out.print("<td>" + activityDate + "</td>");
                                                        out.print("<td id='actvty" + i + "' contenteditable='true' onload=\"clean('actvty" + i + "')\" onkeydown=\"clean('actvty" + i + "')\" onkeyup=\"clean('actvty" + i + "')\">" + activityName + "</td>");
                                                        out.print("<td id='actvtOutcome" + i + "' contenteditable='true' onload=\"clean('actvtOutcome" + i + "')\" onkeydown=\"clean('actvtOutcome" + i + "')\" onkeyup=\"clean('actvtOutcome" + i + "')\">" + activityOutcome + "</td>");
                                                        out.print("<td>" + mentor + "</td>");
                                                        out.print("<td>" + department + "</td>");
                                                        out.print("<td id='axnReq" + i + "' contenteditable='true' onload=\"clean('axnReq" + i + "')\" onkeydown=\"clean('axnReq" + i + "')\" onkeyup=\"clean('axnReq" + i + "')\">" + requiredAction + "</td>");
